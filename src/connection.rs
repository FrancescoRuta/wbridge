    ...
    Self: Sized,
{
    ...
    fn split<'a>(
        ...
        ConnectionWriteHalf<Self::W<'a>>,
        ConnectionReadHalf<Self::R<'a>>,
    );
    fn on_close(self) {}
}
...
pub trait ConnectionManager {
    fn manage(
        self,
        id: u32,
        broadcast_subscriptions: Arc<
            DashMap<u64, DashMap<u32, tokio::sync::mpsc::Sender<bytes::Bytes>>>,
        >,
        connections: Arc<DashMap<u32, tokio::sync::mpsc::Sender<bytes::Bytes>>>,
        conn_tx: tokio::sync::mpsc::Sender<bytes::Bytes>,
        stop_wh_rx: tokio::sync::mpsc::Receiver<tokio::sync::mpsc::Sender<()>>,
        stop_rh_rx: tokio::sync::mpsc::Receiver<tokio::sync::mpsc::Sender<()>>,
        conn_rx: tokio::sync::mpsc::Receiver<bytes::Bytes>,
        stop_tx: tokio::sync::mpsc::Sender<tokio::sync::mpsc::Sender<()>>,
    );
}
...
impl<T: DuplexConnection + Send + 'static> ConnectionManager for StConnectionManager<T> {
    fn manage(
        ...
        connections: Arc<DashMap<u32, tokio::sync::mpsc::Sender<bytes::Bytes>>>,
        conn_tx: tokio::sync::mpsc::Sender<bytes::Bytes>,
        mut stop_wh_rx: tokio::sync::mpsc::Receiver<tokio::sync::mpsc::Sender<()>>,
        mut stop_rh_rx: tokio::sync::mpsc::Receiver<tokio::sync::mpsc::Sender<()>>,
        mut conn_rx: tokio::sync::mpsc::Receiver<bytes::Bytes>,
        stop_tx: tokio::sync::mpsc::Sender<tokio::sync::mpsc::Sender<()>>,
    ) {
        ...
        tokio::spawn(async move {
            ...
            let wh = async move {
                ...
                let _ = w_notify_stop
                    .send(tokio::sync::mpsc::channel::<()>(1).0)
                    .await;
            };
            let rh = async move {
                ...
                let snd = loop {
                    ...
                    let to_conn =
                        u32::from_be_bytes([message[12], message[13], message[14], message[15]]);
                    ...
                    if from_conn == id {
                        if to_conn == 0 {
                            ...
                            let channels = broadcast_subscriptions
                            ...
                            if broken_channels.len() > 0 {
                                if let Some(m) = broadcast_subscriptions.get(&broadcast_channel) {
                                    broken_channels.iter().for_each(|k| {
                                        m.remove(k);
                                    });
                                }
                            }
                ...
                for (channel_id, channel) in broadcast_channels {
                    ...
                    let chs = if let dashmap::mapref::entry::Entry::Occupied(e) =
                        broadcast_subscriptions.entry(channel)
                    {
                        Some(e.remove())
                    } else {
                        None
